...
class Board extends BaseActiveRecord
{
	...
	public static function updateLastData($boardId, $threadId, $threadTitle, 
			$isThread = True)
	{
		$attributes = [];
		...
		if ($isThread)
		{
			$attributes['threads'] = new Expression("[[threads]]+:bp0", 
					[
							":bp0" => 1
					]);
		}
	...
	public static function getParentIds($id)
	{
		...
		$current = Board::findOne([
				'id' => $id
		]);
		
		$parent = Board::findOne([
	...
	public static function createCache()
	{
		...
		foreach ($channels as $row)
		{
			...
			$content .= Board::getCacheItemValue('parent_ids', 
					implode(',', $parentIds));
			...
			$content .= Board::getCacheItemValue('level', count($parentIds) - 1, 
					true);
